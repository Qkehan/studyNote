<?php
    ...
    // 数组元素的值可以是数组；示例：      
    $products = array(
        array("pname"=>"HUAWEI Mate 20","price"=>4500,),  
        array("pname"=>"Iphone XR","price"=>5500,)   
    );    
    echo '<br>';   
    foreach ($products as $value) {
        foreach ($value as $val) {  //第二层里面的key值就是上一层数组的value值。   
            echo "$val<br>";   
        }
    }
    echo "<br>*********<br>";  
    ...
    */
	...
	each($arr);			
	echo current($arr)."<br/>";   //banana
	next($arr);
	echo current($arr)."<br/>";   //orange
	end($arr);
	echo current($arr)."<br/>";   //orange  
    ...
    echo "<br>----------<br>";    
    //
    //8、explode() 返回由字符串组成的数组(相当于把字符串转换为数组)    
    ...
    $arr=explode(",",$str);   //一定会返回数组，只需赋值即可      
    print_r($arr);     //Array ( [0] => 11 [1] => 22 [2] => 33 [3] => 44 [4] => 55 [5] => 66 )     
    echo "<br>----------<br>";     
    //
    //
    $str="西安、咸阳、宝鸡、渭南、汉中、安康、延安、铜川、榆林";   
    echo "explode处理前：".$str."<br>";  
    $arr=explode("、",$str);    
    echo "explode处理后：".print_r($arr)."<br>";  
    foreach($arr as $val){
        echo $val."<br>";    
    }
    //
    //9、implode() 将数组元素连接成字符串（把数组元素转换为字符串）     
    //例：  
    $arr1=array("a","b","c","d");   
    $str=implode("*",$arr1);  
    echo $str;  //a*b*c*d    
