<?php
    ...
    */
    ...
    //
    $a=strtotime("2021-01-01 00:00:00");   
    $b=strtotime(date("Y-m-d H:i:s"));     
    echo $a." "."输出日期："."2021-01-01 00:00:00<br>";  
    echo $b." "."输出日期：".date("Y-m-d H:i:s")."<br>";  
    echo "2021年已经过去：". ceil(($b-$a)/60/60/24) ."天<br>";  
    //
    //
    //计算出今天是当年的第几天，当月的第几天。   
    date_default_timezone_set("PRC");  
    echo "当前日期：".date("Y-m-d")."<br>";  
    echo "当年的第：".date("z")."天<br>";  
    echo "当月的第：".date("j")."<br>";  
    //
    //用getdate输出  
    echo "当前日期：".date("Y-m-d")."<br>";  
    $timer=getdate();  
    print_r("$timer[year] - $timer[month] - $timer[mday] ");
    echo "今天是一年中的第 $timer[yday] 天，是这个月的第 $timer[mday] 天。<br><br><br><br>";  
    //  
    //
    //双11倒计时  
    $today=strtotime(date("Y-m-d H:i:s"));   
    $SPDay=strtotime("2021-11-11 00:00:00");   
    echo "距离2021双11还有". ceil(($SPDay-$today)/60/60/24) ."天！";  
    //
